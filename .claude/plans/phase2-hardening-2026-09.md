...
- Since Task 18: shelf sizes come from `getShelfCounts()` (three HEAD counts), never from loading rows; `/my-shelf` and `/api/shelf/books` page through `getUserBooks(userId, { status | shelfId, limit: SHELF_PAGE_SIZE, offset })`. `public/sw.js` is `ohmyreads-v2` and must stay static-only — never `cache.put` a page, RSC payload or API response.
...
| 18 | my-shelf counts via `head:true`, paginate grid, scope the service worker | 🟡 Medium | Medium | [x] COMPLETE | `app/(app)/my-shelf/page.tsx`, `lib/queries/users.ts`, `components/books/shelf-grid.tsx` (new), `app/api/shelf/books/route.ts` (new), `app/offline/page.tsx` (new), `public/sw.js`, `components/pwa/service-worker-registration.tsx`, `__tests__/lib/queries/users.test.ts` (new), `__tests__/app/api/shelf-books.test.ts` (new) |
| 19 | SEO: canonicals, sitemap/robots, noindex hidden profiles, titles, OG rating source, logo | 🟡 Medium | Medium | [x] COMPLETE | `app/(public)/{books,authors,lists,clubs}/[slug]/page.tsx`, `app/sitemap.ts`, `app/robots.ts`, `app/(public)/users/[username]/page.tsx`, `app/api/og/book/route.tsx`, `app/(public)/page.tsx`, `app/(public)/{trending,recommendations,about,discover}/page.tsx` |
...
**Steps:**
1. [x] Counts: three `head: true` count queries (or one `group by status` RPC) in `Promise.all`; grid paginated at 48 with a "Load more" island; custom shelf via one `user_books` select with `shelf_books!inner(shelf_id)`.
2. [x] `getUserStats`: same count approach.
3. [x] `sw.js`: cache only `/_next/static/**`, fonts, icons and a static `/offline` page; skip requests with `_rsc` or `Accept: text/x-component`, skip responses with `Cache-Control: private` or `no-store`; bump `CACHE_NAME`; delete old caches on activate.
4. [x] Remove the production `console.log` (components are lint-exempt but it is still noise).
5. [x] Create a test user with > 1000 `user_books` rows via SQL (rolled back after) or trust the count queries — document which.

**Verify:**
- [x] Shelf counts match `SELECT status, count(*) … GROUP BY status` for a test user. → Throwaway user seeded with 60 `user_books` (20 per status) and a 5-book custom shelf: `/my-shelf` header "60 books in your collection", the three stat cards 20 / 20 / 20, grid "Load more (48 of 60)", `/api/shelf/books?offset=48` → 12 rows / total 60, `?status=read` → 20 / 20, custom shelf page "5 books in this shelf" and `?shelf=` → 5 / 5 through the join; `SELECT status, count(*) … GROUP BY status` → read 20, reading 20, want_to_read 20. User deleted afterwards (0 residue rows in `auth.users`, `user_books`, `user_shelves`).
- [x] After a deploy, DevTools > Application > Cache Storage contains no HTML/RSC entries. → Checked on a local production build (`next build` + `next start -p 3001`, SW registers only in production) with the Chrome MCP: one cache (ohmyreads-v2) with 2 icons, the manifest, the static /offline page, 2 fonts, 2 stylesheets and 26 hashed chunks after visiting / and a book page; no other HTML and no RSC payload; worker active and controlling. The production deploy repeats this automatically on the next push.
- [x] Gates at baseline: tsc clean, lint 0 errors / 25 warnings, **486 passed + 1 skipped / 45 files** (was 476 / 43).

**Completed Notes:**
- Files modified: `lib/queries/users.ts` — `getShelfCounts(userId)` (three `select("id", { count: "exact", head: true })` in `Promise.all`, `all` = sum because `user_books.status` is CHECK-constrained to the three values; a failed count logs and reads as 0), `SHELF_PAGE_SIZE = 48`, `getUserBooks` gains `shelfId` (`shelf_books!inner(shelf_id)` + `.eq("shelf_books.shelf_id", …)` in the same query, exact count included), `getUserStats` reads the counts instead of every row's status. `app/(app)/my-shelf/page.tsx` — counts, first page and shelf name in one `Promise.all` (was: whole collection loaded, filtered and counted in JS; custom shelf = 4 sequential queries), grid handed to `ShelfGrid`. `components/books/shelf-grid.tsx` (new client island) — renders the page, "Load more (n of total)" appends from `/api/shelf/books` with a dedupe on id and an inline error. `app/api/shelf/books/route.ts` (new GET) — `isForeignOrigin` → 403, `getUser` → 401, status allow-list → 400, `offset` ≥ 0, `limit` clamped to 48. `public/sw.js` — `ohmyreads-v2`; precaches `/offline`, the manifest and the two icons (`allSettled`, so a missing icon cannot block install); fetch handler: GET + same-origin only, ignores `/api/`, `/callback`, `?_rsc=` and `Accept: text/x-component`; navigations are network-only with `/offline` as the fallback; only `/_next/static/`, `/icons/`, `/images/`, `/fonts/` and the manifest are cached (cache-first) and never when the response says `private` / `no-store`; activate deletes every other cache and claims clients. `app/offline/page.tsx` (new, static, `noindex`). `components/pwa/service-worker-registration.tsx` — success `console.log` removed, failure still logged. Tests: `users.test.ts` (5: three HEAD counts + sum, failed count → 0 + log, profile stats from the same counts, status page range + no join, custom shelf inner join + filter) and `shelf-books.test.ts` (5: cross-site 403 before auth, 401, unknown status 400, clamp + pass-through, shelf id + defaults; `@vitest-environment node` because happy-dom strips `Sec-Fetch-Site`).
- Approach taken: page and route share `getUserBooks`, so there is exactly one query shape for "a page of this reader's shelf". The route is a GET read (the `/api/books/search` pattern behind `BookBrowser`) rather than a Server Action: Task 22 is moving reads *out* of `"use server"` files. Verification used the Task 14 probe recipe (magic-link session for a throwaway user) plus `seed` / `cleanup` ops that inserted 60 `user_books` rows from real catalogue ids and a custom shelf, then compared page, route and SQL.
- Deviations from plan: (a) **No > 1000-row user.** The catalogue holds 714 books and `user_books` is unique per (user, book), so a throwaway user cannot exceed PostgREST's 1,000-row cap without inserting hundreds of throwaway books into production. The count path is instead proven at 60 rows against `GROUP BY status` and pinned by unit test; the cap argument is structural: `count: "exact", head: true` returns the SQL count regardless of `max-rows`. (b) `/images/` and `/fonts/` were added to the SW allow-list (the hero WebP and any self-hosted font are reader-independent); `/_next/image` output is *not* cached — it can be large and is already behind the optimizer's 30-day `Cache-Control` from Task 15. (c) The old worker precached `/`, `/login`, `/signup`, `/discover`; none are precached now — they are personalised or dynamic, and the offline fallback is the new static page.
- Issues encountered: a dynamic `select()` string defeats Supabase's column typing, so `getUserBooks` casts through `unknown` (the shape is unchanged for callers). Nothing else.
...
**Steps:**
1. [x] `alternates: { canonical: "/books/" + slug }` etc. on the four dynamic templates (`metadataBase` already set).
2. [x] Sitemap: `.eq("discovery_visible", true)` on profiles; drop `/login`,`/signup`; add the missing static routes, clubs and public lists; `updated_at` for books, newest-book date for authors/lists.
3. [x] Profile page: `robots: { index: false, follow: false }` and no Person JSON-LD when `discovery_visible === false`.
4. [x] `/books`: read `searchParams` (`q`, `genre`, `sort`) server-side and seed `BookBrowser`; `generateMetadata` reflecting the genre; canonical to the param-free URL unless a genre is set.
5. [x] Titles: strip the brand from page-level titles; home uses `title: { absolute: … }`.
6. [x] Organization `logo` → an existing icon route; `og:type: "book"`; word-boundary truncation of descriptions with the text fallback for `og:description`.
7. [x] OG book card + author page: use `local_average_rating`/`local_ratings_count` or label "on Open Library".
8. [x] `robots: { index: false, follow: true }` on `/discover` and `/recommendations`.
9. [x] (carried over from Task 16) Real 404 status for missing books and profiles.

**Verify:**
- [x] `curl -s /books/<slug> | grep canonical` present on all four templates.
- [x] Sitemap XML excludes an opted-out test profile and includes `/trending`.
- [x] No page title contains "OhMyReads" twice.
- [x] Gates at baseline.

**Completed Notes:**
- Files modified: `app/sitemap.ts` (rewritten), `app/(public)/books/(index)/page.tsx` (moved from `books/page.tsx`, now reads `searchParams` + `generateMetadata`), `app/(public)/books/(index)/loading.tsx` (moved), **new** `app/(public)/books/[slug]/layout.tsx` and `app/(public)/users/[username]/layout.tsx`, `app/(public)/{books,authors,lists,clubs}/[slug]/page.tsx`, `app/(public)/users/[username]/{page,followers/page,following/page}.tsx`, `app/(public)/{page,trending,about,recommendations,discover}/page.tsx`, `app/api/og/book/route.tsx`, `components/books/book-browser.tsx`, `lib/validation/search.ts` (`parseBrowseParams`), `lib/utils/format.ts` (`truncateAtWord`). Tests: **new** `__tests__/app/sitemap.test.ts` (7), `__tests__/app/api/og-book-rating.test.ts` (3), `__tests__/lib/utils/format.test.ts` (5), `__tests__/lib/validation/browse-params.test.ts` (4).
- Approach taken: (1) canonicals are relative (`metadataBase` resolves them) and always point at the parameter-free URL, so `/books/x?page=2` → `/books/x`; the user page's absolute `${NEXT_PUBLIC_SITE_URL}` canonical became relative too. (2) Sitemap: `createPublicClient()` instead of the cookie client (a crawler has no cookies, and an admin's cookie could have listed hidden profiles) — side effect: the route is now **statically generated with a 1 h revalidate** (`○ /sitemap.xml 1h`), fine for a sitemap. Profiles filtered `discovery_visible = true AND disabled_at IS NULL AND username IS NOT NULL` (the profile page's own rules); `book_clubs` (visibility public, by slug) and `reading_lists` (visibility public, by id) added; `/login` + `/signup` dropped; `/trending`, `/clubs`, `/community` added; `lastModified` = `updated_at ?? created_at` for books/clubs/lists, newest listed book for authors and for curated lists pinned to `bookSlugs`, and **absent** (not `new Date()`) where there is no real date. All five queries run in one `Promise.all`. (3) Profile: `robots noindex,nofollow` + no Person JSON-LD when `discovery_visible === false`, also on the followers/following pages. (4) `/books`: `parseBrowseParams` accepts `q` (trimmed, ≤200), `genre` only if it is in the genre list the page renders (the DB list — that is what book-page genre links use; `GENRE_OPTIONS` in the API schema is narrower), `sort` from `SORT_OPTIONS`; any filter runs `searchBooks()` server-side and seeds `BookBrowser` (new `initialTotal/Query/Genre/Sort` props; a seeded genre beyond the first ten pills expands the pill row; `history.replaceState` keeps the URL in step on later filter changes); the default view stays the cached `getPopularBooks`. Genre pages get a `"<Genre> Books"` title + `/books?genre=` canonical; `q`/`sort` views canonicalise to `/books`. (5) Titles: trending/recommendations/about lose the brand; home uses `title: { absolute }`. (6) Logo → `/icons/icon-512` (an existing route); `og:type: "book"` with `authors`/`isbn`/`releaseDate`; `truncateAtWord(description, 160)` for both description and `og:description`, falling back to the text template. (7) OG card selects the local pair and prefers it ("from N readers on OhMyReads"), otherwise labels Open Library; the author page header says "on Open Library". (8) `noindex, follow` on `/discover` and `/recommendations`.
- Deviations from plan: (a) **`/discover` and `/recommendations` are deliberately not in the sitemap** although E5 listed them as missing — they are now `noindex`, and submitting a noindex URL is contradictory. (b) Authors outside the top-1000 books and curated lists that match by genre carry **no** `lastModified` rather than a fetched newest-book date: computing it would need per-author/per-list queries in the sitemap; the date comes from the same 1000 book rows at zero extra cost. (c) Canonicals were also added to `/`, `/trending`, `/about` (one line each) beyond the four templates. (d) **Soft 404 needed more than "call notFound()"**: `notFound()` in `generateMetadata` still returned 200 on the production build for both a browser UA and Googlebot (Googlebot is not in Next's `htmlLimitedBots`, so metadata streams; and any `loading.tsx` above the page commits the 200 before the page runs). Fix: a same-segment `layout.tsx` that awaits the React-cached lookup and calls `notFound()` above the Suspense boundary; for books that was still not enough because `app/(public)/books/loading.tsx` wrapped the `[slug]` segment as well, so the index page + its skeleton moved into a `books/(index)/` route group (URL unchanged). The book page keeps its own skeleton: warm TTFB 15–30 ms on `next start`. (e) `/books?q=` searches stay indexable with a `/books` canonical (no `noindex`) — the plan only asked for the canonical rule.
- Issues encountered: (i) The stdin-based edit helper decoded input in the Windows code page and double-encoded `…`/`—` in four files (`â€¦`); fixed, and the `truncateAtWord` tests would catch a repeat. (ii) `.env.local`'s `NEXT_PUBLIC_SITE_URL` carries the Vercel-CLI trailing-newline artifact, so dev sitemap `<loc>`s contain a newline after the host — `new URL()` in `metadataBase` trims it, the raw string in the sitemap does not; the production value lives in Vercel, not in that file. Not changed. (iii) `getPopularBooks` gives no total, so the unfiltered `/books` still shows "20 books found" until a filter is applied — pre-existing. (iv) Live proof for the opted-out case: no production profile has `discovery_visible = false`, so the owner's own profile (`pkw1977`) was flipped hidden by SQL for ~10 s: sitemap users 5 → 4 (pkw1977 absent), profile HTML `noindex, nofollow` with 0 Person JSON-LD, followers page `noindex, nofollow`; restored to `true` and re-verified (`index, follow`, Person JSON-LD back, sitemap 5). Sitemap profile count 5 = SQL count with the same predicate; public clubs 2 = 2; public lists 0. (v) Production build checked: missing book/profile → **404** for browser and Googlebot UAs, `/books?genre=Fantasy` renders the "Fantasy Books" h1 with 119 results and its own canonical, `og:type book`, and no title carries the brand twice across `/`, book, author, list, club, user, trending, about, recommendations, discover, books.
...
| 2026-09-03 | 18 | COMPLETE | `getShelfCounts` (3 HEAD counts in parallel, sum = all) behind `/my-shelf` and `getUserStats`; `getUserBooks` gains `shelfId` (`shelf_books!inner`) and the page fetches counts + first 48 + shelf name in one `Promise.all`; `ShelfGrid` island + GET `/api/shelf/books` (403/401/400 gates, limit clamp); `sw.js` v2: static assets + `/offline` only, skips `_rsc`/`text/x-component`/`private`/`no-store`, deletes old caches; registration log removed. Live: 60-row probe shelf → header 60, cards 20/20/20, page 2 = 12, custom shelf 5, SQL GROUP BY 20/20/20. one cache (ohmyreads-v2) with 2 icons, the manifest, the static /offline page, 2 fonts, 2 stylesheets and 26 hashed chunks after visiting / and a book page; no other HTML and no RSC payload; worker active and controlling. +10 tests: 486 passed + 1 skipped / 45 files. |
| 2026-09-03 | 19 | COMPLETE | Canonicals on every public template (+ home/trending/about); sitemap on the anon client (static, 1 h) with discoverable non-disabled profiles only, public clubs + lists, `updated_at` dates, no `/login` `/signup`, `/trending` `/clubs` `/community` added; hidden profiles `noindex,nofollow` + no Person JSON-LD (also followers/following); `/books` seeded from `?q=&genre=&sort=` with genre titles + canonicals (`parseBrowseParams`); brand-free titles, `title.absolute` on home; logo → `/icons/icon-512`; `og:type book`; `truncateAtWord` descriptions; OG card + author page label the rating source; `noindex,follow` on `/discover` + `/recommendations`; real 404s via `[slug]/layout.tsx` + `[username]/layout.tsx` and the `books/(index)/` route group. +19 tests: 505 passed + 1 skipped / 49 files; build exit 0. |
| | | | |
